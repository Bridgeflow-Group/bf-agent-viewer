...
- Test suite covering identity resolution, scope enforcement, rate limiting, event-chain integrity, sandboxing (both the rlimit and container paths, verified against real spawned processes), the console and its auth flow (real requests against a seeded database, driven through the actual login/enroll/verify HTTP routes, not a bypass), the new CLI commands (real invocations against a real SQLite file, not mocks), alerting (persistence, all three channels against a real local HTTP server / a stubbed SMTP, and a real end-to-end gateway rate-limit rejection verified to actually fire and persist an alert), and a concurrent multi-identity integration test against a real running gateway process

**Not yet built:**

- Alerting beyond the rate-limit rejection case above — the gap-marker/gateway-outage alert (F-042) and passive-discovery alert (ties to F-027) are separate, still-open tasks that will use this same alert channel once built
