...
| --- | --- | --- | --- |
| v0.1.0 | building | Know what your agents are doing | Visibility-first release: agent registry, event collection, activity dashboard, identity/owner/environment metadata, tool/resource activity history, search. No enforcement. Formerly "V1" in the design doc. This is the #1 focus and must not get diluted by compliance or enforcement framing. Real implementation started Sept 22, 2026; every v0.1.0-scoped task is now `done` except T-013 (packaging the app as a single runnable container image) — that's blocked purely by this dev environment's lack of container-registry access, not by unfinished work, and needs verification in a registry-reachable environment (a normal dev machine or CI) before an actual v0.1.0 tag is cut. Kill switch (credential revocation, F-009) and short-lived credentials (F-016) — originally scoped for v0.2.0 below — were pulled forward into v0.1.0 once T-022/T-024 turned out to need each other anyway. See [`status.md`](status.md) for current build status. |
