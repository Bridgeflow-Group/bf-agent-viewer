...
| --- | --- | --- | --- |
| v0.1.0 | building | Know what your agents are doing | Visibility-first release: agent registry, event collection, activity dashboard, identity/owner/environment metadata, tool/resource activity history, search. No enforcement. Formerly "V1" in the design doc. This is the #1 focus and must not get diluted by compliance or enforcement framing. Real implementation started Sept 22, 2026: gateway (identity resolution, scope enforcement, rate limiting, tamper-evident logging) has a working, tested core. Console/dashboard, sandboxed backend spawning hardening, and alerting are not yet built. |
| v0.2.0 | proposed | Stop what's going wrong | Kill switch (credential-revocation based, sub-5-minute target) and credential/session lifecycle management. Formerly "V2." Decided as credential revocation (short-lived, platform-issued credentials with a stop-renewal path) rather than an inline enforcement gateway, to hit that target without the bigger architectural commitment a real-time proxy would require. |
